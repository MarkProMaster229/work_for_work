...
def apply_overrides(scenario: dict, overrides: dict) -> dict:
    ...
    overrides = {
        ...
        # design
        "launch_stage": 2,
        # формат, который шлёт фронт:
        "planes": [{"id": "P1", "raan_deg": 45, "phase_deg": 10}],
    ...
    # Поддерживаем оба формата: "planes" (то, что шлёт фронт)
    # и "plane_overrides" (исторический формат).
    planes_over = overrides.get("planes") or overrides.get("plane_overrides") or []
    if planes_over:
        pmap = {p["id"]: p for p in design["planes"]}
        for po in planes_over:
            pid = po.get("id") or po.get("plane_id")
            if pid not in pmap:
                raise ValueError(f"Unknown plane: {pid}")
            if "raan_deg" in po:
                val = po["raan_deg"]
                if not (0 <= val < 360):
                    raise ValueError(f"raan_deg must be 0..360, got {val}")
                pmap[pid]["raan_deg"] = val
            if "phase_deg" in po:
                val = po["phase_deg"]
                if not (0 <= val < 360):
                    raise ValueError(f"phase_deg must be 0..360, got {val}")
                pmap[pid]["phase_deg"] = val
